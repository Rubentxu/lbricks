...
func TestSingleInput(t *testing.T) {
	...
	RunProc(d)
	for i := 0; i < 10; i++ {
		t.Logf("Meter en canal, %d!", i)
		in <- i
		i2 := <-out
		t.Logf("Sacar del canal, %d!", i2)
		ix2 := i * 2
		if i2 != ix2 {
			t.Errorf("%d != %d", i2, ix2)
		}
...
func (l *locker) OnIn(i int) {
	...
	if i%2 == 0 {
		fmt.Printf("Long process %d\n", i)
		time.Sleep(1000)
		fmt.Printf("Long process finish %d \n", i)
	}
