...
func TestMouseInput(t *testing.T) {
	d := new(EngiFlow.MouseSensor)
	in := make(chan *EngiFlow.MouseSignal)
	out := make(chan *EngiFlow.MouseSignal)
	d.In = in
	d.Out = out
	flow.RunProc(d)
	for i := 0.0; i < 1000000.0; i++ {
		var ms = &EngiFlow.MouseSignal{float32(i * 2.0), float32(i * 2.0), engi.PRESS}
		in <- ms
		i2 := <-out
		ix2 := i * 2
